...
{
    [Microsoft.AspNetCore.Components.Route("api/peliculas/{peliculaId:int}/review")]
    ...
    public class ReviewController: CustomBaseController
    {
        ...
        public async Task<ActionResult> Post(int peliculaId, [FromBody] ReviewCreacionDTO reviewCreacionDTO)
        {
            ...
            // El endpoint está protegido por JWT. El cliente debe enviar "Authorization: Bearer <token>".
            // Obtenemos el Id del usuario desde los claims del token (NameIdentifier) para asociar el review.
        ...
        public async Task<ActionResult> Put(int peliculaId, int reviewId, [FromBody] ReviewCreacionDTO reviewCreacionDTO)
        {
            ...
            if (reviewDB.UsuarioId != usuarioId) { return BadRequest("No tiene permiso de editar este review"); }
